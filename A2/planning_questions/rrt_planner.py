...
class RRTPlanner(object):
    ...
    def steer_towards(self, s_nearest, s_rand, max_radius):
        ...
        # Pick points within the radius
            ...
            a = max_radius/dist(s_nearest, s_rand)
            x = s_nearest.x + a*(s_rand.x-s_nearest.x)
            y = s_nearest.y + a*(s_rand.y-s_nearest.y)
    ...
    def path_is_obstacle_free(self, s_from, s_to):
        ...
        for i in range(max_checks):
            ...
            if dist(s_from, s_to) != 0:
                a = val / dist(s_from, s_to)
            else:
                a = 0

            x = s_from.x + a * (s_to.x - s_from.x)
            y = s_from.y + a * (s_to.y - s_from.y)
